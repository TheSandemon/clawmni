...
async function executeFinger(taskIssue, octokit, db, owner, repo) {
    ...
    try {
        // Each finger execution consumes 3 fuel units (API call cost)
        const fuelCost = 3;
        
        // Consume fuel upfront
        const fuelRef = db.ref('state/fuel_consumed');
        await fuelRef.transaction(current => (current || 0) + fuelCost);
        ...
        if (filesToCommit && filesToCommit.length > 0) {
            ...
            // Reward system - cap at 100
            const dopaRef = db.ref('state/dopamine');
            await dopaRef.transaction(current => Math.min((current || 0) + 15, 100));
        } else {
...
Format: [{"path": "src/example.js", "content": "console.log('hello');"}]`;
        ...
        if (!rawContent) {
            console.error("[Arms] OpenRouter returned empty response or error:", data);
            return null;
        }
...
async function createPullRequest(octokit, owner, repo, taskId, taskTitle, files, db) {
    ...
    // 2. Get current default branch ref to branch off of
